...
#include "qt/Hemis/clickablelabel.h"
...
GameMasterWizardDialog::GameMasterWizardDialog(WalletModel* model, GMModel* _gmModel, ClientModel* _clientModel, QWidget *parent) :
...
void GameMasterWizardDialog::initSummaryPage()
{
...
void GameMasterWizardDialog::moveToNextPage(int currentPos, int nextPos)
{
...
void GameMasterWizardDialog::setSummary()
{
    assert(gmSummary);
    setShortText(ui->labelMainAddr, QString::fromStdString(gmSummary->ownerAddr), 14);
    setShortText(ui->labelPayoutAddr, QString::fromStdString(gmSummary->ownerPayoutAddr), 14);
    setShortText(ui->labelCollateralHash, QString::fromStdString(gmSummary->collateralOut.hash.GetHex()), 20);
    ui->labelCollateralIndex->setText(QString::number(gmSummary->collateralOut.n));
    ui->labelOperatorService->setText(QString::fromStdString(gmSummary->service));
    setShortText(ui->labelOperatorKey, QString::fromStdString(gmSummary->operatorKey), 20);
    if (gmSummary->operatorPayoutAddr) {
        setShortText(ui->labelOperatorPayout, QString::fromStdString(*gmSummary->operatorPayoutAddr), 14);
        ui->labelOperatorPercentage->setText(QString::number(gmSummary->operatorPercentage)+ " %");
    } else {
        ui->labelOperatorPayout->setText(tr("No address"));
    }
    setShortText(ui->labelVoterAddress, QString::fromStdString(gmSummary->votingKey), 14);
}
...
{
    if (input.isEmpty()) {
        const auto addr = walletModel->getNewAddress("dgm_"+label_prefix+"_"+alias);
...
CallResult<std::pair<std::string, CKeyID>> GameMasterWizardDialog::getOrCreateOwnerAddress(const std::string& alias)
{
    return getOrCreateAddress(ui->lineEditOwnerAddress->text(), alias, "owner", walletModel, true);
}

CallResult<std::pair<std::string, CKeyID>> GameMasterWizardDialog::getOrCreatePayoutAddress(const std::string& alias)
{
    return getOrCreateAddress(ui->lineEditPayoutAddress->text(), alias, "payout", walletModel, false);
}

CallResult<std::pair<std::string, CKeyID>> GameMasterWizardDialog::getOrCreateVotingAddress(const std::string& alias)
{
...
bool GameMasterWizardDialog::createGM()
{
    ...
    if (isDeterministic) {
        ...
        auto res = gmModel->createDGM(alias,
            collateralOut,
            addressLabel,
            ipAddress,
            port,
        ...
        gmSummary = std::make_unique<GMSummary>(alias,
            ipAddress + ":" + port,
        ...
        returnStr = tr("Deterministic Gamemaster created! It will appear on your list on the next mined block!");
    } else {
        // Legacy
        ...
        if (!walletModel->getGMCollateralCandidate(collateralOut)) {
            ...
            if (!gmModel->createDGMExternalCollateral(addressLabel,
        ...
        std::string gmPubKeyStr = secret.GetPubKey().GetHash().GetHex();

        gmEntry = gmModel->createLegacyGM(collateralOut, alias, ipAddress, port, gmKeyString, gmPubKeyStr, returnStr);
        if (!gmEntry) return false;
        // Update list
        gmModel->addGm(gmEntry);
        returnStr = tr("Gamemaster created! Wait %1 confirmations before starting it.").arg(gmModel->getGamemasterCollateralMinConf());
    }
    return true;
}

bool GameMasterWizardDialog::validateService()
{
...
bool GameMasterWizardDialog::validateOwner()
{
...
bool GameMasterWizardDialog::validateOperator()
{
    QString operatorKey(ui->lineEditOperatorKey->text());
...
void GameMasterWizardDialog::moveBack(int backPos)
{
...
void GameMasterWizardDialog::onAddrListClicked(const QStringList& types, QLineEdit* edit)
{
